...
- `arg()` / `args()` / `argc()` は**トップレベル専用**。関数内での使用は現在コンパイルエラーにならないが動作は未定義なので書かないこと ([16 章](#16-既知の制限))。引数の値が必要な場合はトップレベルで受けてから関数へ渡す。
...
- 引数は **必ず argv 配列** (`[...]` リテラルまたは `args()`)。先頭要素がコマンド、残りが引数。コマンドは PATH から探索される。
...
スクリプトに渡されたコマンドライン引数へアクセスする。**トップレベル専用** (関数内では使わない — [11 章](#11-関数))。
...
- Windows 引数転送の文字制限に注意: `%` `!` など cmd が解釈する文字を含む引数は化ける可能性がある ([16 章](#16-既知の制限))。
...
- クロージャ、再帰、関数の値渡し以外の引数、`Int` 型パラメータ
- `break` / `continue`
...
Windows では引数が cmd (Batch) の `%*` を経由して PowerShell へ転送されるため、**cmd が特別扱いする文字を含む引数は化ける・失われる可能性があります**。具体的には `%` `!` `"` `^` `&` `|` `<` `>` など。英数字・空白・日本語・ハイフン・ドット等の通常の引数は問題ありません (E2E テストで検証された範囲を使ってください)。

### その他

- `arg()` / `args()` / `argc()` の**関数内使用は現在コンパイルエラーにならないが動作は未定義** (macOS では関数自身の引数を参照してしまう)。仕様どおりトップレベルでのみ使うこと。
- `arg(0)` 以下のインデックスは範囲検査されず、OS 間で意味が一致しない。1 以上のみ使うこと。
- `run([])` (空の argv) は検査を通るが生成物が正しく動かない。必ず 1 要素以上を渡すこと。
- 識別子は ASCII のみ (日本語識別子は不可。文字列データとしての日本語は問題ない)。
- 実行されなかった分岐内でのみ `let` された変数は、実行時に空の値になる ([6 章](#6-変数と-let))。使用前に初期値を代入すること。
