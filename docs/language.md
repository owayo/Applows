...
- 値は符号付き 64bit (`Int`)。範囲は `-9223372036854775808` から `9223372036854775807`。
- 負数は単項マイナスで書く: `let n = -42`

### コンパイラの安全上限

- ブロック・括弧・単項演算などの再帰的なネストは 256 段まで。
- 1 つの式で再帰走査を必要とする演算・呼び出し・リストなどの複合要素は 256 個まで。
- 上限を超えた入力はプロセスを abort せず、分割方法を示すコンパイル診断になる。長い式は複数の `let` 文へ分割する。
...
- `/` は両 OS とも 0 方向へ切り捨てる整数除算 (`7 / 2 == 3`、`-7 / 2 == -3`)。
  ゼロ除算とオーバーフロー時のエラー形態だけは OS 間で異なる ([16 章](#16-既知の制限))。
...
- 戻り値は終了コード (`Int`)。
- コマンドを起動できない場合は両 OS とも 127 を返す。`run(args())` へ空の引数列を
  渡した場合も「コマンド未指定」として 127 を返す。
- **シェルを経由しない**: パイプ `|`、リダイレクト `>`、ワイルドカード `*`、`&&` などのシェル構文は使えない (書いてもただの文字として渡る)。
- 標準出力・標準エラーは画面へそのまま流れる。**stdout を値として使いたいときは `run_capture`** (下記)。

### 標準出力を取得する (run_capture)

```applows
let toplevel = run_capture(["git", "rev-parse", "--show-toplevel"], "")
if toplevel != "" {
  print "repo={toplevel}"
}

# 「成功して空」と「失敗」を区別したいときは default を番兵にする
let branch = run_capture(["git", "branch", "--show-current"], "(unknown)")
```

- `run_capture(argv, default)` は argv を実行し、**終了コードが 0 のときだけ標準出力を返す**。
  起動失敗 (コマンドが無い) と非 0 終了はどちらも `default` を返す。
- 標準エラーは捨てる (画面にも出さない)。
- 出力に加える正規化は 3 つだけ: **NUL と CR を除去**し、**末尾の改行をすべて削る**。
  行間の空行は残る。
- 「成功して出力が空」のときは空文字が返る (`default` ではない)。区別が要るなら `default` に
  番兵を置く。
- `args()` も渡せる。引数が 0 個ならコマンド未指定として実行せず `default` を返す。
  空リテラル `run_capture([], ...)` はコンパイルエラー。
- **戻り値を捨てる文としては書けない**。出力が要らないなら `run` を使う
  (捨てると stdout も stderr も消える紛らわしい処理になるため)。

### 移植性の注意

- 呼び出すコマンド自体の存在は Applows は保証しない。両 OS に存在する実行ファイル (`git`、`curl` など、インストール済みのもの) を使うこと。`ls` / `dir`、`open` / `start` のような OS 固有コマンドは避ける。
- `run_capture` が扱えるのは**外部実行ファイルの標準出力**だけ。PowerShell の cmdlet・エイリアス・シェル組み込みを呼ぶと sh 側と結果が揃わない。
- `run_capture` は出力サイズにも実行時間にも上限を設けない。巨大な出力や終了しないコマンドを渡すと、そのまま待ち続けメモリも消費する。
- 出力に**孤立した CR** (改行を伴わない `\r`。プログレス表示など) が含まれるときだけ結果が OS で異なる: sh は CR を除去し、PowerShell は行区切りとして扱って LF にする。通常のテキスト出力なら差は出ない。
- コマンドを起動できない場合、`run` は両 OS とも 127 を返し、`run_capture` は指定された
  `default` を返す。どちらもスクリプト自体は継続する。
...
| `run_capture(list, default)` | List, Text | Text | 値 | 外部コマンドを実行し標準出力を返す。失敗時は `default` ([12 章](#標準出力を取得する-run_capture)) |
...
ファイルを削除する。ディレクトリは削除できない。存在しないパスは両 OS とも無視する。
...
- **JSON を解析しない**。制御文字除去と末尾空白削除による正規化を除き、既存の中身
  (入れ子・配列・数値・空白) は変形しないので、受け取った JSON をほぼそのまま
  転送したいときに向く
- 値は必ず**文字列**として入る。数値や真偽値、入れ子オブジェクトは入れられない
- 既存の同名キーは**上書きしない** (JSON としては重複キーになる)。重ねないよう呼び出し側で管理する
- 前処理として制御文字の除去と末尾空白の削除を行ってから、先頭の空白を除く最初の文字が
  `{` かつ末尾が `}` かを調べる。この形でない入力 (配列・スカラ・空文字) は、
  **正規化後の文字列をそのまま返す**
- 整形済み JSON を渡すと制御文字である改行が落ちて 1 行になる
...
| 存在しないファイルの `read_text` | macOS は空文字列で継続、Windows は実行時エラーで停止 | 必ず `exists()` / `is_file()` で確認してから読む |
| ゼロ除算 / 整数オーバーフロー | 両 OS で実行時エラーになるが、終了コードとエラー表示が異なる | 入力値を事前に検査する |
...
- **`while` / `for` の本体は、ループ前から在る変数の型を変えられない** (ループは複数回実行されるため型は毎周一定である必要がある)。例: `while n > 0 { let n = "x" }` は不可。ただし `for ... in a to b` の**ループ変数自身**は隠しカウンタで反復するため本体で自由に書き換えてよい (次周でカウンタから再代入され、反復は壊れない = Python/Rust と同じ意味論)。
- **`and` / `or` / `not` の内側に副作用のある呼び出し** (`run` / `run_capture` /
  `read_stdin` / `http_download` / `http_post` / ユーザ関数) は書けない。条件は短絡評価されないため、
  `let c = run([...])` で受けてから `c == 0` を条件に使う ([10 章](#10-条件式))。
