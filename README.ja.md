...
- **防御的なコンパイラ上限** — 過剰なネストや再帰走査される式の複雑さは、スタックオーバーフローで abort せず診断エラーになる
...
- 未対応 (将来拡張): 正規表現、文字列 `replace`、辞書/オブジェクト、クロージャ、再帰、パイプライン、例外処理、非同期/並列。標準出力を値として使う場合は `run_capture` を使用する
